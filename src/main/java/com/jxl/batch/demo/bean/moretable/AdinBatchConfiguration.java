...
public class AdinBatchConfiguration {
    ...
    @StepScope
    @Bean
    public JpaPagingItemReader<ADInfomation> readerAdin(@Value("#{jobParameters['tableID']}") String tableID) {
...
//                            .queryString("select a from ADInfomation a ")
    ...
    @Bean
    public JdbcBatchItemWriter<ADInfomation> writerAdin() {
    ...
    // tag::jobstep[]
    @Bean
    public Step stepAdin(JpaPagingItemReader<ADInfomation> readerAdin, AdinStepListener adinStepListener) {
        return stepBuilderFactory.get("stepAdin")
    ...
    @Bean
    public Job importJobAdin(AdinJobListener adinJobListener, Step stepAdin) {
    	//jobBuilderFactory.get("importUserJobMap") 
    	//创建步骤构建器并初始化其作业存储库和事务管理器。 请注意，如果构建器用于创建@Bean定义，则步骤名称和bean名称可能会有所不同。
        return jobBuilderFactory.get("importJobAdin")
